...
/**
 * ASSET CONFIGURATION
 * To use your own illustration files:
 * 1. Place your files in the root directory.
 * 2. Update the paths below (e.g., growth: './growth.png').
 * 3. If a value is null/empty, the procedural SVG fallback will be used.
 */
export const ILLUSTRATION_ASSETS = {
  growth: '',       // Recommended: 1:1 Aspect Ratio
  journey: '',      // Recommended: 1:1 Aspect Ratio
  protective: '',   // Recommended: 2:3 Aspect Ratio (Portrait)
  connection: '',   // Recommended: 1:1 Aspect Ratio
  emptyState: '',   // Recommended: 1:1 Aspect Ratio
  milestone: '',    // Recommended: 1:1 Aspect Ratio
  inclusion: '',    // Recommended: 1:1 Aspect Ratio
};
...
/**
 * Internal helper to decide whether to render an image or the procedural fallback.
 */
const BaseIllustration: React.FC<{
  assetKey: keyof typeof ILLUSTRATION_ASSETS;
  className?: string;
  ariaLabel: string;
  fallback: React.ReactNode;
}> = ({ assetKey, className, ariaLabel, fallback }) => {
  const customAsset = ILLUSTRATION_ASSETS[assetKey];
